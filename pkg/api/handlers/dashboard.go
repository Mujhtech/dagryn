...
func (h *Handler) GetDashboardOverview(w http.ResponseWriter, r *http.Request) {
	...
	recentRuns, err := h.store.Runs.GetRecentRunsAcrossProjects(ctx, projectIDs, 7)
	if err != nil {
	...
	projectStats, err := h.store.Runs.GetProjectStats(ctx, projectIDs, 7)
	if err != nil {
	...
	for uid := range userIDs {
		u, err := h.store.Users.GetByID(ctx, uid)
		if err == nil && u != nil {
