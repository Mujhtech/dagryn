...
Examples:
  ...
  dagryn init --remote           # Create/link project on remote server`,
		RunE: func(cmd *cobra.Command, args []string) error {
			return runInit(flags, forceInit, interactive, templateName, listTemplates, updateGitignore, skipGitignore, remoteSetup, projectName)
		},
	}

	cmd.Flags().BoolVarP(&forceInit, "force", "f", false, "overwrite existing config file")
	cmd.Flags().BoolVarP(&interactive, "interactive", "i", false, "interactively select project type")
	cmd.Flags().StringVar(&templateName, "template", "", "force specific template (go, rust, python, node, java, ruby, php, elixir, swift, cpp, generic)")
	cmd.Flags().BoolVar(&listTemplates, "list-templates", false, "list available templates and exit")
	cmd.Flags().BoolVar(&updateGitignore, "gitignore", false, "add .dagryn/ to .gitignore")
	cmd.Flags().BoolVar(&skipGitignore, "no-gitignore", false, "skip .gitignore handling (no warnings)")
	cmd.Flags().BoolVar(&remoteSetup, "remote", false, "create or link project on remote server")
	cmd.Flags().StringVar(&projectName, "project-name", "", "project name for remote setup (default: directory name)")

	return cmd
}

func runInit(flags *cli.Flags, forceInit, interactive bool, templateName string, listTemplates, updateGitignore, skipGitignore, remoteSetup bool, projectName string) error {
	...
	// Handle .gitignore
	handleGitignore(projectRoot, updateGitignore, skipGitignore)
	...
	if shouldSetupRemote {
		if err := setupRemoteProject(projectRoot, projectName); err != nil {
...
func handleGitignore(projectRoot string, updateGitignore, skipGitignore bool) {
	if skipGitignore {
...
func setupRemoteProject(projectRoot string, projectName string) error {
	...
	if createNew {
		// Create new project
		config, err := createNewProject(ctx, apiClient, projectRoot, projectName)
		if err != nil {
			return err
		}
	...
	if err := cli.SyncWorkflowToRemote(ctx, apiClient, projectRoot, projectConfig.ProjectID); err != nil {
...
func createNewProject(ctx context.Context, apiClient *client.Client, projectRoot string, projectName string) (*client.ProjectConfig, error) {
