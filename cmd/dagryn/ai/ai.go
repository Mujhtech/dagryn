...
// NewCmd creates the AI command.
func NewCmd(flags *cli.Flags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "ai",
		Short: "AI-powered CI analysis tools",
	}
	cmd.AddCommand(newAIAnalyzeCmd(flags))
	return cmd
}

func newAIAnalyzeCmd(flags *cli.Flags) *cobra.Command {
	var (
		runID       string
		projectRoot string
		mode        string
...
configuration from dagryn.toml. Requires AI to be enabled in the config.`,
  ...
  dagryn ai analyze --mode summarize_and_suggest`,
		RunE: func(cmd *cobra.Command, args []string) error {
			return runAIAnalyze(flags, runID, projectRoot, mode, backendMode)
		},
	}
...
func runAIAnalyze(flags *cli.Flags, runIDStr, projectRoot, mode, backendMode string) error {
