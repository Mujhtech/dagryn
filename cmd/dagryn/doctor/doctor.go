...
// NewCmd creates the doctor command.
...
remote server connectivity, Git version, and plugin health.`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			return runDoctor(cmd, args, flags)
		},
	}
}

func runDoctor(cmd *cobra.Command, args []string, flags *cli.Flags) error {
	...
	if err != nil {
		log.Errorf("Config:    %s not found or invalid", flags.CfgFile)
		failed++
	} else {
		errs := config.Validate(cfg)
		if len(errs) > 0 {
			log.Warnf("Config:    %s has %d validation error(s)", flags.CfgFile, len(errs))
			warned++
		} else {
			log.Successf("Config:    %s (%d tasks)", flags.CfgFile, len(cfg.Tasks))
			passed++
		}
	}
