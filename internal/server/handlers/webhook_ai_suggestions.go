...
func (h *Handler) detectSuggestionAcceptance(ctx context.Context, project *models.Project, branch string, commits []GitHubPushCommit) {
	...
	suggestions, err := h.aiRepo.ListPostedSuggestionsByProjectAndBranch(ctx, project.ID, branch)
	...
	for _, commit := range commits {
		...
		for _, file := range modifiedFiles {
			...
			for _, s := range matched {
				if err := h.aiRepo.UpdateSuggestionStatus(ctx, s.ID, models.AISuggestionStatusAccepted, nil, nil); err != nil {
