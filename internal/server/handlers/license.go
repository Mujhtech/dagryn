...
func (h *Handler) GetLicenseStatus(w http.ResponseWriter, r *http.Request) {
	...
	resp := LicenseStatusResponse{
		Mode:     "self_hosted",
		Edition:  string(license.EditionCommunity),
		Licensed: false,
		Seats:    3,
...
func buildFeatureMap(gate *license.FeatureGate) map[string]bool {
	features := []license.Feature{
		license.FeatureContainerExecution,
		license.FeaturePriorityQueue,
		license.FeatureSSO,
		license.FeatureAuditLogs,
		license.FeatureCustomRBAC,
		license.FeatureMultiCluster,
		license.FeatureDashboardFull,
		license.FeatureCloudCache,
	}
...
func buildLicenseLimits(claims *license.Claims) LicenseUsage {
	usage := LicenseUsage{}
