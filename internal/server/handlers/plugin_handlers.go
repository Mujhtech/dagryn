...
func (h *Handler) GetPluginByPublisherName(w http.ResponseWriter, r *http.Request) {
	publisher := strings.TrimSpace(strings.ToLower(chi.URLParam(r, "publisher")))
...
func (h *Handler) ListProjectPlugins(w http.ResponseWriter, r *http.Request) {
	...
	workflows, err := h.workflows.ListByProjectWithTasks(ctx, projectID)
	if err != nil {
...
func (h *Handler) UninstallPlugin(w http.ResponseWriter, r *http.Request) {
	...
	pluginName := strings.TrimSpace(chi.URLParam(r, "pluginName"))
	if pluginName == "" {
		_ = response.BadRequest(w, r, errors.New("plugin name is required"))
		return
	}
